...
def get_default_wikibase():
    default_wikibase_url = settings.BASE_REST_URL.split("/w/rest.php")[0]
...
class Client:
    ...
    def rest_endpoint_url(self):
        return self.token.wikibase.base_endpoint_url
...
class Wikibase(models.Model):
    ...
    oauth_client_id = models.CharField(max_length=128, null=True, blank=True)
    oauth_client_secret = models.CharField(max_length=128, null=True, blank=True)

    @property
    def oauth_client(self):
        global OAUTH

        if not hasattr(OAUTH, self.identifier):
            OAUTH.register(
                name=self.identifier,
                client_id=self.oauth_client_id,
                client_secret=self.oauth_client_secret,
                access_token_url=f"{self.rest_endpoint_url}/oauth2/access_token",
                authorize_url=f"{self.rest_endpoint_url}/oauth2/authorize",
            )
        return getattr(OAUTH, self.identifier)
...
class Token(models.Model):
    ...
    def refresh(self):
        ...
        try:
            new_token = self.wikibase.oauth_client.fetch_access_token(
                grant_type="refresh_token", refresh_token=self.refresh_token
            )
            self.value = newtoken["access_token"]
